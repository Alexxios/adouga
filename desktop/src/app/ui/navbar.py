...
from src.app.ui.network_indicator import NetworkIndicator


class ModernNavbar(tk.Frame):
    """Modern navigation bar with page links, theme toggle, and user controls."""
    ...
    def _create_right_nav(self, backend_url: str):
        """Create right side with theme toggle, settings, and profile."""
        ...
        self._theme_toggle = tk.Label(
            right_nav,
            text="\u263D",  # crescent moon — dark mode active
            bg=ModernTheme.SURFACE_ELEVATED,
            fg=ModernTheme.TEXT_SECONDARY,
            font=(ModernTheme.FONT_FAMILY, 16),
            cursor="hand2",
        )
        self._theme_toggle.pack(side=tk.LEFT, padx=(0, 15))
    ...
    def _toggle_theme(self) -> None:
        ...
        self._theme_toggle.config(
            text="\u2600" if new_theme == "light" else "\u263D",
        )
        if self._on_theme_change:
            self._on_theme_change()

    # ---- Recolor after theme switch -----------------------------------

    def update_view(self) -> None:
        """Reapply theme colours to the navbar widget tree."""
        ModernTheme.recolor_widget_tree(self)
        # Re-apply active-page styling (PRIMARY may have changed)
